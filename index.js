...
app.delete("/delete/:id", async (req, res) => {
   ...
   app.post("/addtoy", async (req, res) => {
    ...
    // app.get("/mytoys/:sortcode/:email", async (req, res) => {
    //   let cursor = toys.find({ selleremail: req.params.email }).sort({ price: parseInt(req.params.sortcode) });
    //   let result = await cursor.toArray();
    //   res.send(result);
    // });
    ...
    //   users.push(data)
    //   userCollections.insertOne(data)
    ...
    //     },
    //   };
    //   let result = await userCollections.updateOne(filter, updateDoc, options);
    //   res.send(JSON.stringify(result))
    // })

    // app.get("/users", async (req, res) => {
    //   let cursor = userCollections.find();
    //   let result = await cursor.toArray();
    //   res.send(result)
    // })
    ...
    //   let cursor = userCollections.findOne({ _id: new ObjectId(userid) });
    //   let result = await cursor
    //   res.send(result)
    // })
    ...
    //   let deleteData = await userCollections.deleteOne(query);
    //   if (deleteData.deletedCount == 1) {
    //     let indexToDelete = users.findIndex(item => item._id == deleteId);
    //     if (indexToDelete > -1) {
    //       users.splice(indexToDelete, 1);
